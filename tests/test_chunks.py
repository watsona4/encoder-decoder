...
def test_finish_drive_single_chunk_with_chunk_aad(monkeypatch, tmp_path):
    client, sid = _client_with_cookie()
    _store_tokens(sid)
    ...
    plaintext = b"single chunk with aad"
    iv = os.urandom(12)
    aes_key = AESGCM.generate_key(bit_length=256)
    aesgcm = AESGCM(aes_key)
    cipher = aesgcm.encrypt(iv, plaintext, b"\x00\x00\x00\x00")
    ...
    header = {
        ...
        "mime": "application/octet-stream",
        "iv": base64.b64encode(iv).decode("ascii"),
        "iv_base": base64.b64encode(iv).decode("ascii"),
        "chunk_size": len(plaintext),
        "chunk_count": 1,
        "last_chunk_bytes": len(plaintext),
        "wrapped_key": base64.b64encode(wrapped_key).decode("ascii"),
    }
    ...
    def _fake_drive_upload(*args, **kwargs):
        called["hit"] = True
        called["content"] = Path(kwargs.get("file_path") or args[2]).read_bytes()
        return True, {"id": "gid", "name": "aad.bin"}
    ...
    js = resp.get_json()
    assert js["ok"] is True
    assert js["name"] == "aad.bin"
    assert called["hit"] is True
    assert called["content"] == plaintext
